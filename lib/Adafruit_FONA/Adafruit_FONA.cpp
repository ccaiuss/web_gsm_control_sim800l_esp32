...
uint8_t Adafruit_FONA::getBaudrateStatus(void) {
  uint16_t status;

  if (! sendParseReply(F("AT+IPR?"), F("+IPR: "), &status, ',', 1)) return 0;
...
boolean Adafruit_FONA::callPhone(char *number) {
  ...
  strncpy(sendbuff+3, number, min<int>(30, strlen(number)));
...
void IRAM_ATTR Adafruit_FONA::onIncomingCall() {
...
unsigned long timeOutCall = millis();
  readline();
  if(prog_char_strcmp(replybuffer, (prog_char*)F("RING")) ) {
    flushInput();
    readline();
	//if(millis() - timeOutCall  > 6000 ){ break;}
  }
 //|| prog_char_strstr(replybuffer, PSTR("+CMTI: "))
...
// printouts!
boolean Adafruit_FONA::readSMS(char *smsNr, uint8_t i, char *smsbuff,
  ...
  DEBUG_PRINTLN(replybuffer);
  boolean result = parseReplyQuoted(F("+CMGR:"), smsNr, 250, ',', 1);

  
  if (! parseReply(F("+CMGR:"), &thesmslen, ',', 11) || !result) {
    *readlen = 0;
    return false;
  }
  ...
  uint16_t thelen = min<uint16_t>(maxlen, strlen(replybuffer));
  ...
  if (! sendCheckReply(sendcmd, ok_reply)) {
    *readlen = 0;
    return false;
      ...
      uint16_t lentocopy = min<uint16_t>(maxlen-1, strend - p);
...
boolean Adafruit_FONA::getTime(char *buff, uint16_t maxlen) {
  ...
  uint16_t lentocopy = min<uint16_t>(maxlen-1, strlen(p));
...
uint8_t Adafruit_FONA::getGPS(uint8_t arg, char *buffer, uint8_t maxbuff) {
  ...
  uint8_t len = max<uint8_t>(maxbuff-1, strlen(p));
